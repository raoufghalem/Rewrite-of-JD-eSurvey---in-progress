...
<title>答案结果回显 - Answer result echo</title>
							...
							<div class="r-qu-body-content">
								<c:choose>
									<%--是非题 - True or false --%>
														...
														<ul class="quItem-ul ">
															<c:choose>
																<c:when test="${en.anRadio.quItemId eq quItem.id }">
																	...
																	<li>${quItem.optionName }<c:if test="${quItem.isNote eq 1 }">&nbsp;&nbsp; 说明 - Instructions ：<input type="text" name="text_qu_${en.quType }_${en.id }_${quItem.id }"  value="${en.anRadio.otherText }" /> </c:if></li>
																</c:when>
																<c:otherwise>
																	<li class="${quItem.isNote eq 1 ?'quItem-ul01li':''}"><input type="radio" name="qu_${en.quType }_${en.id }" value="${quItem.id }"/></li>
																	<li>${quItem.optionName }<c:if test="${quItem.isNote eq 1 }">&nbsp;&nbsp; 说明 - Instructions：<input type="text" name="text_qu_${en.quType }_${en.id }_${quItem.id }" /> </c:if></li>
																</c:otherwise>
															</c:choose>
														</ul>
													</li>
										...
										<%--复合多选 - Composite multiple selection --%>
														...
														<c:forEach items="${en.quCheckboxs }" var="quItem">
															...
															<c:forEach items="${en.anCheckboxs }" var="anCk" varStatus="iStatus" >
																...
																<c:when test="${anCk.quItemId eq quItem.id }">
																	...
																	<li>${quItem.optionName }<c:if test="${quItem.isNote eq 1 }">&nbsp;&nbsp; 说明 - Instructions：<input type="text" name="text_qu_${en.quType }_${en.id }_${quItem.id}"  value="${anCk.otherText }"/> </c:if> </li>
																	<c:set var="isBreak" value="1"></c:set>
																</c:when>
																<c:when test="${(fn:length(en.anCheckboxs) eq iStatus.count ) && (isBreak eq 0)}">
										...
										<c:when test="${en.quType eq 'SCORE' }">
											...
											<table class="qusTable" width="90%" >
												<tr>
													<th width="80">行/分 - Line/minute</th>
										...
										<%--矩阵单选题 - Matrix single choice--%>
										<c:when test="${en.quType eq 'CHENRADIO' }">
										<input type="hidden" name="qu_${en.quType }_${en.id }" value="item_qu_${en.quType }_${en.id }_" />
											<table class="qusTable" width="90%" >
												<tr>
													<th width="80">行/列 - Row/column</th>
										...
										<%--复合矩阵单选题 - Compound matrix single choice--%>
													...
													<tr>
														...
														<c:forEach items="${en.columns }" var="enCol">
															...
															<select name="sel_item_qu_${en.quType }_${en.id }_${enRow.id}_${enCol.id}">
																	<option>-请选择 - please choose -</option>
										...
										<c:otherwise>
											<!-- 题型开发中 - Question type development .. -->
										</c:otherwise>
								</c:choose>
							</div>
						</div>
